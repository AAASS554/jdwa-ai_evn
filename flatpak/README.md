...
This directory contains the Flatpak manifest (`com.jdwa.ai.evn`) for JDWA 记得晚安, used to convert the generated `.deb` artifact into an installable `.flatpak` package via CI or local builds.
...
```bash
cp "$(find src-tauri/target/release/bundle -name '*.deb' | head -n 1)" flatpak/jdwa-ai-evn.deb
```

3) Build the local Flatpak repository and export the `.flatpak`:

```bash
flatpak-builder --force-clean --user --disable-cache --repo flatpak-repo flatpak-build flatpak/com.jdwa.ai.evn.yml
flatpak build-bundle --runtime-repo=https://flathub.org/repo/flathub.flatpakrepo flatpak-repo JDWA-AI-Evn-Linux.flatpak com.jdwa.ai.evn
```

4) Install and run:

```bash
flatpak install --user ./JDWA-AI-Evn-Linux.flatpak
flatpak run com.jdwa.ai.evn
```
...
If you prefer minimal permissions (e.g., for Flathub submission or security concerns), you can replace `--filesystem=home` in `flatpak/com.jdwa.ai.evn.yml` with more precise grants:

```yaml
  - --filesystem=~/.jdwa:create
...
If you plan to publish on Flathub or want stricter permission control, adjust the `finish-args` in `flatpak/com.jdwa.ai.evn.yml` accordingly.
