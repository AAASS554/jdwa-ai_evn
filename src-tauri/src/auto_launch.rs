...
/// 将 `/path/to/JDWA 记得晚安.app/Contents/MacOS/JDWA 记得晚安` 转换为 `/path/to/JDWA 记得晚安.app`
#[cfg(target_os = "macos")]
...
mod tests {
    ...
    fn test_get_macos_app_bundle_path_valid() {
        let exe_path =
            std::path::Path::new("/Applications/JDWA 记得晚安.app/Contents/MacOS/JDWA 记得晚安");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(
            result,
            Some(std::path::PathBuf::from("/Applications/JDWA 记得晚安.app"))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_with_spaces() {
        ...
        assert_eq!(
            ...
            Some(std::path::PathBuf::from(
                "/Users/test/My Apps/JDWA 记得晚安.app"
            ))
        );
    }
    ...
    fn test_get_macos_app_bundle_path_not_in_bundle() {
        let exe_path = std::path::Path::new("/usr/local/bin/jdwa-ai-evn");
        let result = get_macos_app_bundle_path(exe_path);
    ...
    fn test_get_macos_app_bundle_path_dev_build() {
        ...
        let exe_path = std::path::Path::new("/Users/dev/project/target/debug/jdwa-ai-evn");
        let result = get_macos_app_bundle_path(exe_path);
        assert_eq!(result, None);
    }
}
