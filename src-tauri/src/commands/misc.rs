...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/AAASS554/jdwa-ai_evn/releases/latest",
            None::<String>,
        )
...
pub(crate) fn launch_terminal_running(command_line: &str, label: &str) -> Result<(), String> {
    ...
    let (script_file, script_content) = {
        let file = temp_dir.join(format!("jdwa_{}_{}.sh", label, pid));
        let content = format!(
            r#"#!/bin/bash
trap 'rm -f "{script_path}"' EXIT
echo "[jdwa] Starting: {cmd}"
echo ""
{cmd}
echo ""
echo "[jdwa] Command exited. Press any key to close."
read -n 1 -s
"#,
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let bat_file = temp_dir.join(format!("jdwa_{}_{}.bat", label, pid));
        let content = format!(
            "@echo off\r\necho [jdwa] Starting: {cmd}\r\necho.\r\n{cmd}\r\necho.\r\necho [jdwa] Command exited. Press any key to close.\r\npause >nul\r\ndel \"%~f0\" >nul 2>&1\r\n",
            cmd = command_line,
        );
