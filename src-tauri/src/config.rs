...
pub const APP_CONFIG_DIR_NAME: &str = ".jdwa";
pub const LEGACY_APP_CONFIG_DIR_NAME: &str = ".cc-switch";
pub const APP_DB_FILE_NAME: &str = "jdwa.db";
...
///   且不一定等于用户目录，可能导致 `.jdwa/jdwa.db` 路径变化，从而“看起来像数据丢失”。
///
/// ## 测试隔离
///
...
/// 获取旧版应用配置目录路径 (~/.cc-switch)
pub fn get_legacy_app_config_dir() -> PathBuf {
    get_home_dir().join(LEGACY_APP_CONFIG_DIR_NAME)
}

fn copy_file_if_missing(source: &Path, target: &Path) -> Result<bool, AppError> {
...
fn copy_dir_contents_if_missing(source: &Path, target: &Path) -> Result<bool, AppError> {
    if !source.exists() || target.exists() {
        return Ok(false);
    }
    ...
    for entry in fs::read_dir(source).map_err(|e| AppError::io(source, e))? {
        ...
        if file_type.is_dir() {
            ...
            let _ = copy_file_if_missing(&source_path, &target_path)?;
        }
    }
...
/// 首次启动 JDWA 版时复制旧版 CC Switch 数据，旧目录保持不变。
pub fn migrate_legacy_cc_switch_data() -> Result<bool, AppError> {
    if crate::app_store::get_app_config_dir_override().is_some() {
        log::info!("检测到自定义配置目录，跳过默认目录迁移");
        return Ok(false);
    }

    let legacy_dir = get_legacy_app_config_dir();
    ...
    for dir in ["skills", "skill-backups", "backups", "env-backups"] {
        migrated |= copy_dir_contents_if_missing(&legacy_dir.join(dir), &target_dir.join(dir))?;
    }

    if migrated {
        log::info!(
            "已将旧版 CC Switch 数据从 {} 复制到 {}，旧目录未删除",
            legacy_dir.display(),
            target_dir.display()
        );
    }
