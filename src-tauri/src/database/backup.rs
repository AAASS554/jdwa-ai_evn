...
use crate::config::{get_app_config_dir, APP_DB_FILE_NAME};
...
const JDWA_SQL_EXPORT_HEADER: &str = "-- JDWA SQLite 导出";
...
impl Database {
    ...
    fn validate_cc_switch_sql_export(sql: &str) -> Result<(), AppError> {
        ...
        if trimmed.starts_with(JDWA_SQL_EXPORT_HEADER)
        ...
        Err(AppError::localized(
            ...
            "仅支持导入由 JDWA 记得晚安导出的 SQL 备份文件。",
            "Only SQL backups exported by JDWA are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join(APP_DB_FILE_NAME);
        if !db_path.exists() {
            return Ok(None);
        }
