...
fn test_parse_valid_claude_deeplink() {
    let url = "jdwa://v1/import?resource=provider&app=claude&name=Test%20Provider&homepage=https%3A%2F%2Fexample.com&endpoint=https%3A%2F%2Fapi.example.com&apiKey=sk-test-123&icon=claude";
...
#[test]
fn test_parse_legacy_ccswitch_deeplink() {
    let url = "ccswitch://v1/import?resource=provider&app=claude&name=Legacy";

    let request = parse_deeplink_url(url).unwrap();

    assert_eq!(request.version, "v1");
    assert_eq!(request.app, Some("claude".to_string()));
    assert_eq!(request.name, Some("Legacy".to_string()));
}
