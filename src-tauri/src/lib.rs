...
fn is_supported_deeplink_scheme(url_str: &str) -> bool {
    url_str.starts_with("jdwa://") || url_str.starts_with("ccswitch://")
}

/// 统一处理 jdwa:// / ccswitch:// 深链接 URL
///
/// - 解析 URL
...
fn handle_deeplink_url(
    ...
    if !is_supported_deeplink_scheme(url_str) {
        return false;
    }
...
pub fn run() {
    // 设置 panic hook，在应用崩溃时记录日志到 <app_config_dir>/crash.log（默认 ~/.jdwa/crash.log）
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            if let Err(e) = crate::config::migrate_legacy_cc_switch_data() {
                log::warn!("迁移旧版 CC Switch 数据失败，继续使用 JDWA 默认目录: {e}");
            }
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/jdwa.log）
            {
                ...
                let log_file_path = log_dir.join("jdwa.log");
            ...
            let db_path = app_config_dir.join(crate::config::APP_DB_FILE_NAME);
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to the app data directory using the app identifier.
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/jdwa-ai-evn-handler.desktop").exists())
                        .unwrap_or(true);
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including jdwa.db)\n\
            2) If you see “database version is newer”, please upgrade JDWA\n\
